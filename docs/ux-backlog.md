...
| U1-27 | 有更新时贴纸齿轮被整个改成更新入口，期间贴纸上没有打开设置的入口 | `Sticker.tsx:985-994` | 齿轮保持=设置，更新做独立红点徽章 | 已修（实拍回退：独立成第 6 颗按钮后底栏放不下，置顶被挤出可视区。改两档：红点回齿轮角标做提示、齿轮点击恒为设置；`ready` 时才出一条横条直达更新窗，可关且离开 ready 复位） |
...
> 点了不再写 folded 状态。
>
> **第三轮复核**：又查出 1 条必改——7C-2 尾新加的题面折叠条**没有清理路径**：题面被
> 「仅收起」后若在终端答掉、broker 超时或被别处结算，折叠条永远挂着，点开是一张已死的
> 卡去 resolve 一个不存在的请求（屏幕识别那条有 revealTerminalAttention(null) 兜底，
> 题面这条没有对应物）。修法：useApprovalChannel 暴露 liveQuestionId（轮询 + cleared
> 事件维护的「此刻真的还挂着的 requestId」），折叠条据它判活，换会话一并清；补了回归
> 用例。同处顺带：两条折叠条可以同时在场，文案已分「已收起提示」/「已收起提问」。
> 另修两条低：跨会话消息块的溢出判定加 ResizeObserver（此前只在文本变化时量一次，
> 窗口宽度变化后会出现「短消息挂着展开钮」）；测试的选项匹配改为按 DOM 认主标签
> （.dd-label 首个文本节点），前缀匹配会放过日后新增的同前缀名。
>
> **7T-4 根治（实拍新证）**：用户截图显示中途附加目录必失败——`/add-dir` 的路径是
> **Meowo 自己**加的引号（session_command.rs），而 claude 的 slash 命令不做引号剥离，
> 于是引号进了路径、还让它不再以盘符开头被当成相对路径拼到 cwd 后面。原注释写的
...
>
> **T-15 切回终端页的残帧（实拍）**：切回终端页后底部的输入框边框与状态行缺一截，只剩
> 半个字符（`66% ctx` 只画出一个 `6`），拖一下窗口就好。既有的自愈全都指望「TUI 收到
> SIGWINCH 整屏重绘」——可见时那发无条件 resize 的注释写着「换来切回终端页必然自愈」，
> 但**尺寸没变时后端 last_size 同值短路、根本不发信号**，那句承诺从来没兑现过。真正的
> 缺口是宿主侧没有任何本地重绘：隐藏期间容器尺寸为 0，WebGL 画布与纹理图集失效，
> xterm 缓冲区里的数据是对的、屏幕上却是残帧。修法：可见时 clearTextureAtlas() +
> refresh(0, rows-1)，不惊动 TUI。补用例并红→绿验证。
> 关联：7T-3 的首帧字号缓存让 applyTermStyle 在命中时提前返回，顺带移除了「settings
> 到达后必然 fit 一次」这个隐式重绘，改过字号的用户因此更容易撞上——这条修复把它一并
> 兜住了。
