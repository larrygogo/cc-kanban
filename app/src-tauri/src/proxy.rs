...
///
/// 拉起 agent 时**不要直接用它**——请用 [`crate::terminal::launch_env_for_profile`]，
/// 它会在代理之上补上 `CLAUDE_CONFIG_DIR` 之类的账号隔离变量（恢复会话的账号推导
/// 由 `prepare_session_for_active_profile` 完成，取到的实际账号同样走它）。
///
