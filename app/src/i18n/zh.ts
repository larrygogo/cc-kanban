...
export const zh = {
  ...
  sticker: {
    ...
    focusBackground: "这是 Agent 自己派出的后台会话，没有可跳转的终端窗口。打开对话页即可查看画面并发消息。",
  ...
  chat: {
    ...
    sendBackgroundSession: "这是 Agent 自己派出的后台会话：可以在这里发消息、看画面、结束它，但终端里的交互操作（斜杠菜单、审批选择）要回到派出它的终端处理。",
    sendBackgroundKeysMovedYou:
      "后台会话不接受终端按键，已把你带到对话页——从这里发消息，Agent 收得到。",
    sendBackgroundQueued: "已投递给后台作业。它若处于手动模式或正在收尾就不会处理这条消息——内容留在输入框里，去终端确认后可以直接重发。",
    sendBackgroundNoAttachments: "后台会话暂不支持附件，请只发文字。",
    inputLocked: "终端正在等待选择——先用上方卡片或终端页处理，再继续输入",
    send: "发送",
    sending: "发送中…",
    attach: "添加图片或文件",
    ...
    resumePermissionTip: "本次恢复使用的权限模式；选了会记住，之后的恢复自动沿用",
    ...
    terminalStreamStalled: "终端输出流已停滞超过 30 秒（会话仍在运行）：ConPTY 管道疑似卡死，画面不会自行恢复。点「结束终端」后重新启动即可继续，对话记录不受影响。",
    queuedInterjections: (n: number) => `${n} 条插话已排队,当前回合结束后处理`,
    queuedAttachmentOnly: "(附件)",
    interjectNow: "立即插话",
    interjectNowTip: "中断当前回合,立刻处理排队的插话",
    queuedDismiss: "移除这条回执",
    queuedDismissTip: "只移除这条回执,消息仍会执行",
    interruptAndSendTip: "Ctrl+Enter:中断当前回合并发送(Enter 是排队)",
    interruptNow: "中断",
    interruptNowTip: "中断当前回合,不发送消息",
    attentionDismiss: "仅收起",
    attentionDismissTip: "只收起卡片,不向终端发送按键",
    ...
    modeUnreachable: "没能切到该模式——当前账号或启动参数下它可能不可用",
    modelProbing: "正在读取模型清单…",
    modelListUnavailable: (command: string) => `读不到这个 Agent 的模型清单，点这里去终端页用 ${command} 切换`,
  ...
  account: {
    ...
    switchCoverage: "切换仅对之后新建 / 恢复的会话生效，正在运行的会话保持原账号",
    ...
    mergeProfileConfirm: (name: string) =>
      `将账号「${name}」合并进默认账号？它的会话与历史数据会并入默认账号（默认账号已有的文件不受影响），此操作不可逆。`,
    deleteProfile: "删除账号",
